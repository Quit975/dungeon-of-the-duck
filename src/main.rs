...
impl State {
    fn new() -> Self {
        ...
        .for_each(|pos| spawn_random_monster(&mut ecs, &mut rng, pos));
...
impl GameState for State {
    fn tick(&mut self, ctx: &mut BTerm) {
        ...
        match current_turn_state {
            ...
            TurnState::EnemyTurn => {
                self.enemy_systems.execute(&mut self.ecs, &mut self.resources)
            }
        }
...
fn main() -> BError {
    let context : BTerm = BTermBuilder::new()
        .with_title("Dungeon of the Duck")
        .with_fps_cap(30.0)
